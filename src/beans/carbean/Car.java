...
public class Car {
	...
	public static void main(String[] args) {
		// Create an Owner
		OwnerBean anOwner = new OwnerBean(1, "Rich", "Bell", "male");

		// Create a CarBean
		CarBean aCar = new CarBean(2005, "Honda", "Civic", "silver", anOwner);

		// Print the properties of Rich's Car
		...
		// Change the properties of Rich's Car
		aCar.setYear(2015);
		...
		CarBean richsJSONCar = convertJsonToCar("test.json");

		// Print the car's properties as read from the JSON file
		System.out.println("According to the JSON file, "
				+ richsJSONCar.getOwner() + " will drive a "
				+ richsJSONCar.getColor().toLowerCase() + " "
				+ richsJSONCar.getYear() + " " + richsJSONCar.getMake() + " "
				+ richsJSONCar.getModel() + ".");
	}
	...
	private static CarBean convertJsonToCar(String aFileName) {
		...
		try {
			...
			//richsJSONCar = new CarBean(parsedJSONMap);
		} catch (Exception e) {
			e.printStackTrace();
		}
